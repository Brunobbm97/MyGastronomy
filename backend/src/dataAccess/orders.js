...
export default class OrdersDataAccess {
    ...
    async addOrder(orderData) {
        const { items, ...orderDateRest } = orderData

        orderDateRest.createdAt = new Date()
        orderDateRest.pickupStatus = 'Pending'
        orderDateRest.userId = new ObjectId(orderDateRest.userId)

        const newOrder = await Mongo.db
            .collection(collectionName)
            .insertOne(orderDateRest)
    ...
    async updateOrder(orderId, orderData) {
        if (orderData.password) {
            ...
            crypto.pbkdf2(orderData.password, salt, 310000, 16, 'sha256', async (err, hashedPassword) => {
                if (err) {
                    throw new Error('Error during hashing password')
                }
                orderData = { ...orderData, password: hashedPassword, salt }
